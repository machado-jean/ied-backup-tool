...
def test_backup_service_accepts_custom_project_type(tmp_path: Path) -> None:
    ...
    assert [path.name for path in atu.glob("*.zip")] == [
        "FAKE-V1_SEL-751_2026-06-18_10h30_COLABORADOR EXEMPLO_TAF.zip"
    ]
...
def test_backup_service_accepts_pcm600_project_type(tmp_path: Path) -> None:
    ...
    assert [result.status for result in results] == ["stored"]
    assert [path.name for path in atu.glob("*.zip")] == [
        "PCM600-V2.10_SE-DDD_2026-06-19_12h30_COLABORADOR EXEMPLO_TAF.zip"
    ]
...
def test_backup_service_accepts_pcm600_apcmp_project_type(tmp_path: Path) -> None:
    ...
    assert [result.status for result in results] == ["stored"]
    assert [path.name for path in atu.glob("*.zip")] == [
        "PCM600-V2.13_SE-DDD_2026-06-19_12h30_COLABORADOR EXEMPLO_TAF.zip"
    ]
