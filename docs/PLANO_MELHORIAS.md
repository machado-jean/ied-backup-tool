...
| `v1.17.2` | Temas e confirmação de publicação | Adiciona temas claro/escuro com preferência persistida, uniformiza geometria, contraste e contornos da interface e reforça o publicador com relatório de verificações, CI obrigatório da tag e confirmação humana imediatamente antes do GitHub Release. |
| `v1.17.3` | Diagnóstico concorrente e normalização da SE | Substitui o marcador compartilhado por arquivos individuais com UUID, PID, horário de criação do processo e caminho do executável; evita falsos alertas entre instâncias simultâneas, adiciona retenção automática dos marcadores e converte espaços no identificador do projeto em hífens (`SE CCC` → `SE-CCC`). |
